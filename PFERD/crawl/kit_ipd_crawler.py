...
class KitIpdCrawler(HttpCrawler):
    ...
    async def _download_file(self, parent: PurePath, file: KitIpdFile) -> None:
        ...
        if not maybe_dl:
            ...
            self._output_dir.report.add_custom_value(
                ETAG_KEY_PATTERN.format(element_path),
                self._output_dir.prev_report.get_custom_value(ETAG_KEY_PATTERN.format(element_path))
            )
            return
